...
namespace ExitGames.UtilityScripts
{
	public class PlayerRoomIndexing : PunBehaviour
	{
		...
		public void Awake()
		{
			if (instance != null)
			{
				DConsole.LogError("Existing instance of PlayerRoomIndexing found. Only One instance is required at the most. Please correct and have only one at any time.");
			}
			instance = this;
		...
		private void AssignIndex(PhotonPlayer player)
		{
			...
			{
				_indexesLUT = _indexes as Dictionary<int, int>;
				list = new List<bool>(new bool[_indexesLUT.Keys.Max() + 1]);
				foreach (KeyValuePair<int, int> item in _indexesLUT)
				{
