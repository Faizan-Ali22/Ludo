...
namespace ExitGames.Demos.DemoAnimator
{
	public class Launcher : PunBehaviour
	{
		...
		private void Awake()
		{
			if (loaderAnime == null)
			{
				DConsole.LogError("<Color=Red><b>Missing</b></Color> loaderAnime Reference.", this);
			}
		...
		{
			DConsole.Log("Region:" + PhotonNetwork.networkingPeer.CloudRegion);
			if (isConnecting)
			{
				LogFeedback("OnConnectedToMaster: Next -> try to Join Random Room");
				DConsole.Log("DemoAnimator/Launcher: OnConnectedToMaster() was called by PUN. Now this client is connected and could join a room.\n Calling: PhotonNetwork.JoinRandomRoom(); Operation will fail if no room found");
				PhotonNetwork.JoinRandomRoom();
			}
		}
		...
		public override void OnPhotonRandomJoinFailed(object[] codeAndMsg)
		{
			...
			DConsole.Log("DemoAnimator/Launcher:OnPhotonRandomJoinFailed() was called by PUN. No random room available, so we create one.\nCalling: PhotonNetwork.CreateRoom(null, new RoomOptions() {maxPlayers = 4}, null);");
		...
		public override void OnDisconnectedFromPhoton()
		{
			...
			DConsole.LogError("DemoAnimator/Launcher:Disconnected");
		...
		public override void OnJoinedRoom()
		{
			...
			DConsole.Log("DemoAnimator/Launcher: OnJoinedRoom() called by PUN. Now this client is in a room.\nFrom here on, your game would be running. For reference, all callbacks are listed in enum: PhotonNetworkingMessage");
			if (PhotonNetwork.room.PlayerCount == 1)
			{
				DConsole.Log("We load the 'Room for 1' ");
