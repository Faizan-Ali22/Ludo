...
public class ChatGui : MonoBehaviour, IChatClientListener
{
	...
	public void Start()
	{
		...
		if (!flag)
		{
			DConsole.LogError("You need to set the chat app ID in the PhotonServerSettings file in order to continue.");
		}
	}
	...
	public void Connect()
	{
		...
		chatClient.Connect(chatAppSettings.AppId, "1.0", new Photon.Chat.AuthenticationValues(UserName));
		ChannelToggleToInstantiate.gameObject.SetActive(value: false);
		DConsole.Log("Connecting as: " + UserName);
	...
	private void SendChatMessage(string inputLine)
	{
		...
		if (inputLine[0].Equals('\\'))
		{
			...
			{
				DConsole.Log("The command '" + array[0] + "' is invalid.");
			}
		}
		else if (flag)
		{
	...
	public void OnSubscribed(string[] channels, bool[] results)
	{
		...
		DConsole.Log("OnSubscribed: " + string.Join(", ", channels));
	...
	private void InstantiateChannelButton(string channelName)
	{
		...
		{
			DConsole.Log("Skipping creation for an existing channel toggle.");
			return;
		}
	...
	public void OnUnsubscribed(string[] channels)
	{
		foreach (string text in channels)
		{
			if (channelToggles.ContainsKey(text))
			{
				...
				DConsole.Log("Unsubscribed from channel '" + text + "'.");
			...
			{
				DConsole.Log("Can't unsubscribe from channel '" + text + "' because you are currently not subscribed to it.");
			}
		}
	}
	...
	public void OnPrivateMessage(string sender, object message, string channelName)
	{
		...
		{
			DConsole.Log("Message with byte[].Length: " + array.Length);
		}
	...
	{
		DConsole.LogWarning("status: " + $"{user} is {status}. Msg:{message}");
	...
	{
		DConsole.LogFormat("OnUserSubscribed: channel=\"{0}\" userId=\"{1}\"", channel, user);
	}

	public void OnUserUnsubscribed(string channel, string user)
	{
		DConsole.LogFormat("OnUserUnsubscribed: channel=\"{0}\" userId=\"{1}\"", channel, user);
	}
	...
	public void AddMessageToSelectedChannel(string msg)
	{
		...
		{
			DConsole.Log("AddMessageToSelectedChannel failed to find channel: " + selectedChannelName);
		}
		else
		{
	...
	public void ShowChannel(string channelName)
	{
		...
		{
			DConsole.Log("ShowChannel failed to find channel: " + channelName);
			return;
		}
		selectedChannelName = channelName;
		CurrentChannelText.text = channel.ToStringMessages();
		DConsole.Log("ShowChannel: " + selectedChannelName);
