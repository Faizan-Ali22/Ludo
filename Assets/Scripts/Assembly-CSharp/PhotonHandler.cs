...
internal class PhotonHandler : MonoBehaviour
{
	...
	protected void Update()
	{
		...
		{
			DConsole.LogError("NetworkPeer broke!");
		}
		else
		{
	...
	internal IEnumerator PingAvailableRegionsCoroutine(bool connectToBest)
	{
		while (PhotonNetwork.networkingPeer.AvailableRegions == null)
		{
			...
			{
				DConsole.LogError("Call ConnectToNameServer to ping available regions.");
				yield break;
			}
			DConsole.Log(string.Concat("Waiting for AvailableRegions. State: ", PhotonNetwork.connectionStateDetailed, " Server: ", PhotonNetwork.Server, " PhotonNetwork.networkingPeer.AvailableRegions ", (PhotonNetwork.networkingPeer.AvailableRegions != null).ToString()));
			yield return new WaitForSeconds(0.25f);
		}
		if (PhotonNetwork.networkingPeer.AvailableRegions == null || PhotonNetwork.networkingPeer.AvailableRegions.Count == 0)
		{
			DConsole.LogError("No regions available. Are you sure your appid is valid and setup?");
			yield break;
		}
		...
		DConsole.Log(string.Concat("Found best region: '", bestRegion.Code, "' ping: ", bestRegion.Ping, ". Calling ConnectToRegionMaster() is: ", connectToBest.ToString()));
		if (connectToBest)
		{
