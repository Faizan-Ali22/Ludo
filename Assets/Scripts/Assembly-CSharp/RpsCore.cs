...
public class RpsCore : PunBehaviour, IPunTurnManagerCallbacks
{
	...
	{
		DConsole.Log("OnTurnBegins() turn: " + turn);
	...
	{
		DConsole.Log("OnTurnCompleted: " + obj);
		CalculateWinAndLoss();
		UpdateScores();
		OnEndTurn();
	}
	...
	{
		DConsole.Log(string.Concat("OnPlayerMove: ", photonPlayer, " turn: ", turn, " action: ", move));
		throw new NotImplementedException();
	}

	public void OnPlayerFinished(PhotonPlayer photonPlayer, int turn, object move)
	{
		DConsole.Log(string.Concat("OnTurnFinished: ", photonPlayer, " turn: ", turn, " action: ", move));
	...
	public void OnTurnTimeEnds(int obj)
	{
		if (!IsShowingResults)
		{
			DConsole.Log("OnTurnTimeEnds: Calling OnTurnCompleted");
			OnTurnCompleted(-1);
		}
	}
	...
	public override void OnJoinedRoom()
	{
		...
		{
			DConsole.Log("Waiting for another player");
		}
	}

	public override void OnPhotonPlayerConnected(PhotonPlayer newPlayer)
	{
		DConsole.Log("Other player arrived");
	...
	{
		DConsole.Log("Other player disconnected! " + otherPlayer.ToStringFull());
	}
