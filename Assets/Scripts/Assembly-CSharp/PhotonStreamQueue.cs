...
public class PhotonStreamQueue
{
	...
	private void BeginWritePackage()
	{
		...
		{
			DConsole.LogWarning("The number of objects sent via a PhotonStreamQueue has to be the same each frame");
			DConsole.LogWarning("Objects in List: " + m_Objects.Count + " / Sample Count: " + m_SampleCount + " = " + m_Objects.Count / m_SampleCount + " != " + m_ObjectsPerSample);
		}
		m_IsWriting = true;
		m_SampleCount++;
