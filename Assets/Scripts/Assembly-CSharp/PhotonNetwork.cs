...
public static class PhotonNetwork
{
	...
	public static string CurrentCluster
	{
		get
		{
			...
			return networkingPeer.CurrentCluster;
		}
	}
	...
	public static bool offlineMode
	{
		...
		set
		{
			...
			if (value && connected)
			{
				DConsole.LogError("Can't start OFFLINE mode while connected!");
				return;
			}
	...
	public static bool autoCleanUpPlayerObjects
	{
		...
		set
		{
			if (room != null)
			{
				DConsole.LogError("Setting autoCleanUpPlayerObjects while in a room is not supported.");
			}
			else
			{
	...
	public static int sendRateOnSerialize
	{
		...
		set
		{
			if (value > sendRate)
			{
				DConsole.LogError("Error: Can not set the OnSerialize rate higher than the overall SendRate.");
				value = sendRate;
			}
	...
	public static bool CrcCheckEnabled
	{
		...
		set
		{
			...
			{
				DConsole.Log("Can't change CrcCheckEnabled while being connected. CrcCheckEnabled stays " + networkingPeer.CrcEnabled);
			}
		}
	}
	...
	public static bool ConnectUsingSettings(string gameVersion)
	{
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("ConnectUsingSettings() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (PhotonServerSettings == null)
		{
			DConsole.LogError("Can't connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: PhotonServerSettings");
			return false;
		}
		if (PhotonServerSettings.HostType == ServerSettings.HostingOption.NotSet)
		{
			DConsole.LogError("You did not select a Hosting Type in your PhotonServerSettings. Please set it up or don't use ConnectUsingSettings().");
			return false;
		}
		...
		if (offlineMode)
		{
			DConsole.LogWarning("ConnectUsingSettings() disabled the offline mode. No longer offline.");
		}
		offlineMode = false;
	...
	public static bool ConnectToMaster(string masterServerAddress, int port, string appID, string gameVersion)
	{
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("ConnectToMaster() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (offlineMode)
		{
			offlineMode = false;
			DConsole.LogWarning("ConnectToMaster() disabled the offline mode. No longer offline.");
		}
		if (!isMessageQueueRunning)
		{
			isMessageQueueRunning = true;
			DConsole.LogWarning("ConnectToMaster() enabled isMessageQueueRunning. Needs to be able to dispatch incoming messages.");
		}
	...
	public static bool Reconnect()
	{
		...
		{
			DConsole.LogWarning("Reconnect() failed. It seems the client wasn't connected before?! Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("Reconnect() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (offlineMode)
		{
			offlineMode = false;
			DConsole.LogWarning("Reconnect() disabled the offline mode. No longer offline.");
		}
		if (!isMessageQueueRunning)
		{
			isMessageQueueRunning = true;
			DConsole.LogWarning("Reconnect() enabled isMessageQueueRunning. Needs to be able to dispatch incoming messages.");
		}
		networkingPeer.IsUsingNameServer = false;
	...
	public static bool ReconnectAndRejoin()
	{
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("ReconnectAndRejoin() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (offlineMode)
		{
			offlineMode = false;
			DConsole.LogWarning("ReconnectAndRejoin() disabled the offline mode. No longer offline.");
		}
		if (string.IsNullOrEmpty(networkingPeer.GameServerAddress))
		{
			DConsole.LogWarning("ReconnectAndRejoin() failed. It seems the client wasn't connected to a game server before (no address).");
			return false;
		}
		if (networkingPeer.enterRoomParamsCache == null)
		{
			DConsole.LogWarning("ReconnectAndRejoin() failed. It seems the client doesn't have any previous room to re-join.");
			return false;
		}
		if (!isMessageQueueRunning)
		{
			isMessageQueueRunning = true;
			DConsole.LogWarning("ReconnectAndRejoin() enabled isMessageQueueRunning. Needs to be able to dispatch incoming messages.");
		}
		networkingPeer.IsUsingNameServer = false;
		networkingPeer.IsInitialConnect = false;
	...
	public static bool ConnectToBestCloudServer(string gameVersion)
	{
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("ConnectToBestCloudServer() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (PhotonServerSettings == null)
		{
	...
	public static bool ConnectToRegion(CloudRegionCode region, string gameVersion, string cluster = null)
	{
		if (networkingPeer.PeerState != PeerStateValue.Disconnected)
		{
			DConsole.LogWarning("ConnectToRegion() failed. Can only connect while in state 'Disconnected'. Current state: " + networkingPeer.PeerState);
			return false;
		}
		if (PhotonServerSettings == null)
		{
			DConsole.LogError("Can't connect: ServerSettings asset must be in any 'Resources' folder as: PhotonServerSettings");
			return false;
		}
		if (PhotonServerSettings.HostType == ServerSettings.HostingOption.OfflineMode)
		{
		...
		{
			DConsole.Log("ConnectToRegion: " + region);
	...
	private static bool VerifyCanUseNetwork()
	{
		...
		DConsole.LogError("Cannot send messages when not connected. Either connect to Photon OR use offline mode!");
		return false;
	}
	...
	public static bool CreateRoom(string roomName, RoomOptions roomOptions, TypedLobby typedLobby, string[] expectedUsers)
	{
		if (offlineMode)
		{
			if (offlineModeRoom != null)
			{
				DConsole.LogError("CreateRoom failed. In offline mode you still have to leave a room to enter another.");
				return false;
			}
		...
		{
			DConsole.LogError("CreateRoom failed. Client is not on Master Server or not yet ready to call operations. Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
			return false;
		}
	...
	public static bool JoinRoom(string roomName, string[] expectedUsers)
	{
		if (offlineMode)
		{
			if (offlineModeRoom != null)
			{
				DConsole.LogError("JoinRoom failed. In offline mode you still have to leave a room to enter another.");
				return false;
			}
		...
		if (networkingPeer.Server != ServerConnection.MasterServer || !connectedAndReady)
		{
			DConsole.LogError("JoinRoom failed. Client is not on Master Server or not yet ready to call operations. Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
			return false;
		}
		if (string.IsNullOrEmpty(roomName))
		{
			DConsole.LogError("JoinRoom failed. A roomname is required. If you don't know one, how will you join?");
			return false;
		}
		EnterRoomParams enterRoomParams = new EnterRoomParams();
		enterRoomParams.RoomName = roomName;
	...
	public static bool JoinOrCreateRoom(string roomName, RoomOptions roomOptions, TypedLobby typedLobby, string[] expectedUsers)
	{
		if (offlineMode)
		{
			if (offlineModeRoom != null)
			{
				DConsole.LogError("JoinOrCreateRoom failed. In offline mode you still have to leave a room to enter another.");
				return false;
			}
			EnterOfflineRoom(roomName, roomOptions, createdRoom: true);
			return true;
		...
		if (networkingPeer.Server != ServerConnection.MasterServer || !connectedAndReady)
		{
			DConsole.LogError("JoinOrCreateRoom failed. Client is not on Master Server or not yet ready to call operations. Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
			return false;
		}
		if (string.IsNullOrEmpty(roomName))
		{
			DConsole.LogError("JoinOrCreateRoom failed. A roomname is required. If you don't know one, how will you join?");
			return false;
		}
		typedLobby = typedLobby ?? (networkingPeer.insideLobby ? networkingPeer.lobby : null);
		EnterRoomParams enterRoomParams = new EnterRoomParams();
	...
	public static bool JoinRandomRoom(Hashtable expectedCustomRoomProperties, byte expectedMaxPlayers, MatchmakingMode matchingType, TypedLobby typedLobby, string sqlLobbyFilter, string[] expectedUsers = null)
	{
		if (offlineMode)
		{
			if (offlineModeRoom != null)
			{
				DConsole.LogError("JoinRandomRoom failed. In offline mode you still have to leave a room to enter another.");
				return false;
			}
		...
		if (networkingPeer.Server != ServerConnection.MasterServer || !connectedAndReady)
		{
			DConsole.LogError("JoinRandomRoom failed. Client is not on Master Server or not yet ready to call operations. Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
			return false;
		}
		typedLobby = typedLobby ?? (networkingPeer.insideLobby ? networkingPeer.lobby : null);
	...
	public static bool ReJoinRoom(string roomName)
	{
		if (offlineMode)
		{
			DConsole.LogError("ReJoinRoom failed due to offline mode.");
			return false;
		}
		if (networkingPeer.Server != ServerConnection.MasterServer || !connectedAndReady)
		{
			DConsole.LogError("ReJoinRoom failed. Client is not on Master Server or not yet ready to call operations. Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
			return false;
		}
		if (string.IsNullOrEmpty(roomName))
		{
			DConsole.LogError("ReJoinRoom failed. A roomname is required. If you don't know one, how will you join?");
			return false;
		}
		EnterRoomParams enterRoomParams = new EnterRoomParams();
		enterRoomParams.RoomName = roomName;
	...
	public static bool LeaveRoom(bool becomeInactive = true)
	{
		...
		if (room == null)
		{
			DConsole.LogWarning("PhotonNetwork.room is null. You don't have to call LeaveRoom() when you're not in one. State: " + connectionStateDetailed);
		}
		else
		{
	...
	public static bool RaiseEvent(byte eventCode, object eventContent, bool sendReliable, RaiseEventOptions options)
	{
		...
		{
			DConsole.LogWarning("RaiseEvent() failed. Your event is not being sent! Check if your are in a Room and the eventCode must be less than 200 (0..199).");
			return false;
		}
	...
	public static int AllocateSceneViewID()
	{
		if (!isMasterClient)
		{
			DConsole.LogError("Only the Master Client can AllocateSceneViewID(). Check PhotonNetwork.isMasterClient!");
			return -1;
		}
	...
	public static void UnAllocateViewID(int viewID)
	{
		...
		{
			DConsole.LogWarning($"UnAllocateViewID() should be called after the PhotonView was destroyed (GameObject.Destroy()). ViewID: {viewID} still found in: {networkingPeer.photonViewList[viewID]}");
		}
	}
	...
	public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
	{
		...
		{
			DConsole.LogError("Failed to Instantiate prefab: " + prefabName + ". Client should be in a room. Current connectionStateDetailed: " + connectionStateDetailed);
			return null;
		}
		...
		if (value == null)
		{
			DConsole.LogError("Failed to Instantiate prefab: " + prefabName + ". Verify the Prefab is in a Resources folder (and not in a subfolder)");
			return null;
		}
		if (value.GetComponent<PhotonView>() == null)
		{
			DConsole.LogError("Failed to Instantiate prefab:" + prefabName + ". Prefab must have a PhotonView component.");
			return null;
		}
	...
	public static GameObject InstantiateSceneObject(string prefabName, Vector3 position, Quaternion rotation, byte group, object[] data)
	{
		if (!connected || (InstantiateInRoomOnly && !inRoom))
		{
			DConsole.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". Client should be in a room. Current connectionStateDetailed: " + connectionStateDetailed);
			return null;
		}
		if (!isMasterClient)
		{
			DConsole.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". Client is not the MasterClient in this room.");
			return null;
		}
		if (!UsePrefabCache || !PrefabCache.TryGetValue(prefabName, out var value))
		{
		...
		if (value == null)
		{
			DConsole.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". Verify the Prefab is in a Resources folder (and not in a subfolder)");
			return null;
		}
		if (value.GetComponent<PhotonView>() == null)
		{
			DConsole.LogError("Failed to InstantiateSceneObject prefab:" + prefabName + ". Prefab must have a PhotonView component.");
			return null;
		}
		Component[] photonViewsInChildren = value.GetPhotonViewsInChildren();
		...
		if (array == null)
		{
			DConsole.LogError("Failed to InstantiateSceneObject prefab: " + prefabName + ". No ViewIDs are free to use. Max is: " + MAX_VIEW_IDS);
			return null;
		}
	...
	public static bool CloseConnection(PhotonPlayer kickPlayer)
	{
		...
		{
			DConsole.LogError("CloseConnection: Only the masterclient can kick another player.");
			return false;
		}
		if (kickPlayer == null)
		{
			DConsole.LogError("CloseConnection: No such player connected!");
			return false;
		}
	...
	public static bool SetMasterClient(PhotonPlayer masterClientPlayer)
	{
		if (!inRoom || !VerifyCanUseNetwork() || offlineMode)
		{
			...
			{
				DConsole.Log("Can not SetMasterClient(). Not in room or in offlineMode.");
			}
			return false;
		}
	...
	public static void Destroy(PhotonView targetView)
	{
		...
		{
			DConsole.LogError("Destroy(targetPhotonView) failed, cause targetPhotonView is null.");
		}
	}
	...
	public static void DestroyPlayerObjects(PhotonPlayer targetPlayer)
	{
		if (player == null)
		{
			DConsole.LogError("DestroyPlayerObjects() failed, cause parameter 'targetPlayer' was null.");
		}
	...
	public static void DestroyPlayerObjects(int targetPlayerId)
	{
		if (VerifyCanUseNetwork())
		{
			...
			{
				DConsole.LogError("DestroyPlayerObjects() failed, cause players can only destroy their own GameObjects. A Master Client can destroy anyone's. This is master: " + isMasterClient);
			}
		}
	}
	...
	public static void DestroyAll()
	{
		...
		{
			DConsole.LogError("Couldn't call DestroyAll() as only the master client is allowed to call this.");
		}
	}
	...
	public static void RemoveRPCs(PhotonPlayer targetPlayer)
	{
		if (VerifyCanUseNetwork())
		{
			...
			{
				DConsole.LogError("Error; Only the MasterClient can call RemoveRPCs for other players.");
			}
			else
			{
	...
	internal static void RPC(PhotonView view, string methodName, PhotonTargets target, bool encrypt, params object[] parameters)
	{
		...
		if (room == null)
		{
			DConsole.LogWarning("RPCs can only be sent in rooms. Call of \"" + methodName + "\" gets executed locally only, if at all.");
		}
		else if (networkingPeer != null)
		{
			if (room.serverSideMasterClient)
		...
		{
			DConsole.LogWarning("Could not execute RPC " + methodName + ". Possible scene loading in progress?");
		}
	}
	...
	internal static void RPC(PhotonView view, string methodName, PhotonPlayer targetPlayer, bool encrpyt, params object[] parameters)
	{
		...
		if (player == null)
		{
			DConsole.LogError("RPC can't be sent to target PhotonPlayer being null! Did not send \"" + methodName + "\" call.");
		}
		if (networkingPeer != null)
		{
