...
public class CullArea : MonoBehaviour
{
	...
	private void CreateCellHierarchy()
	{
		if (!IsCellCountAllowed())
		{
			...
			{
				DConsole.LogError("There are too many cells created by your subdivision options. Maximum allowed number of cells is " + (250 - FIRST_GROUP_ID) + ". Current number of cells is " + CellCount + ".");
				return;
			}
			Application.Quit();
		}
