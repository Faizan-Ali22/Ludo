...
public class PickupItemSyncer : Photon.MonoBehaviour
{
	...
	{
		DConsole.Log("Joined Room. isMasterClient: " + PhotonNetwork.isMasterClient.ToString() + " id: " + PhotonNetwork.player.ID);
	...
	public void AskForPickupItemSpawnTimes()
	{
		...
		if (PhotonNetwork.playerList.Length < 2)
		{
			DConsole.Log("Cant ask anyone else for PickupItem spawn times.");
		...
		DConsole.Log("No player left to ask");
		IsWaitingForPickupInit = false;
	}
	...
	public void RequestForPickupItems(PhotonMessageInfo msgInfo)
	{
		...
		{
			DConsole.LogError("Unknown player asked for PickupItems");
		}
		else
		{
	...
	private void SendPickedUpItems(PhotonPlayer targetPlayer)
	{
		...
		{
			DConsole.LogWarning("Cant send PickupItem spawn times to unknown targetPlayer.");
			return;
		}
		...
		foreach (PickupItem pickupItem in array)
		{
			...
			{
				DConsole.Log(pickupItem.ViewID + " respawn: " + pickupItem.TimeOfRespawn + " timeUntilRespawn: " + num2 + " (now: " + PhotonNetwork.time + ")");
				list.Add(pickupItem.ViewID);
				list.Add((float)num2);
			}
		}
		DConsole.Log("Sent count: " + list.Count + " now: " + time);
	...
	public void PickupItemInit(double timeBase, float[] inactivePickupsAndTimes)
	{
		...
		for (int i = 0; i < inactivePickupsAndTimes.Length / 2; i++)
		{
			...
			DConsole.Log(photonView.viewID + " respawn: " + num3 + " timeUntilRespawnBasedOnTimeBase:" + num2 + " SecondsBeforeRespawn: " + component.SecondsBeforeRespawn);
