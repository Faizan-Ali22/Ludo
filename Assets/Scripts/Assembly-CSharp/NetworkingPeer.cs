...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	{
		DConsole.LogError("Avoid using this directly. Thanks.");
		return false;
	}
	...
	public bool ReconnectToMaster()
	{
		if (AuthValues == null)
		{
			DConsole.LogWarning("ReconnectToMaster() with AuthValues == null is not correct!");
	...
	public bool ReconnectAndRejoin()
	{
		if (AuthValues == null)
		{
			DConsole.LogWarning("ReconnectAndRejoin() with AuthValues == null is not correct!");
			AuthValues = new AuthenticationValues();
		}
		AuthValues.Token = tokenCache;
	...
	public bool Connect(string serverAddress, ServerConnection type)
	{
		...
		{
			DConsole.LogWarning("Ignoring Connect() because app gets closed. If this is an error, check PhotonHandler.AppQuits.");
			return false;
		}
		if (State == ClientState.Disconnecting)
		{
			DConsole.LogError("Connect() failed. Can't connect while disconnecting (still). Current state: " + PhotonNetwork.connectionStateDetailed);
			return false;
		}
	...
	protected internal void SetupProtocol(ServerConnection serverType)
	{
		...
		if (AuthMode == AuthModeOption.AuthOnceWss)
		{
			if (serverType != ServerConnection.NameServer)
			{
				...
				{
					DConsole.LogWarning("Using PhotonServerSettings.Protocol when leaving the NameServer (AuthMode is AuthOnceWss): " + PhotonNetwork.PhotonServerSettings.Protocol);
				}
			...
			else
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
				{
					DConsole.LogWarning("Using WebSocket to connect NameServer (AuthMode is AuthOnceWss).");
				}
		...
		if (base.TransportProtocol != connectionProtocol)
		{
			if (PhotonNetwork.logLevel >= PhotonLogLevel.ErrorsOnly)
			{
				DConsole.LogWarning(string.Concat("Protocol switch from: ", base.TransportProtocol, " to: ", connectionProtocol, "."));
			}
	...
	public override void Disconnect()
	{
		if (base.PeerState == PeerStateValue.Disconnected)
		{
			if (!PhotonHandler.AppQuits)
			{
				DConsole.LogWarning($"Can't execute Disconnect() while not connected. Nothing changed. State: {State}");
			}
		}
		else
		{
	...
	public void ChangeLocalID(int newID)
	{
		if (LocalPlayer == null)
		{
			DConsole.LogWarning($"LocalPlayer is null or not in mActors! LocalPlayer: {LocalPlayer} mActors==null: {mActors == null} newID: {newID}");
		}
	...
	protected internal void LocalCleanupAnythingInstantiated(bool destroyInstantiatedGameObjects)
	{
		...
		{
			DConsole.LogWarning("It seems some instantiation is not completed, as instantiation data is used. You should make sure instantiations are paused when calling this method. Cleaning now, despite this.");
		}
	...
	private void GameEnteredOnGameServer(OperationResponse operationResponse)
	{
		if (operationResponse.ReturnCode != 0)
		{
			...
			case 227:
				...
				{
					DConsole.Log("Create failed on GameServer. Changing back to MasterServer. Msg: " + operationResponse.DebugMessage);
				}
			...
			case 226:
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
				{
					DConsole.Log("Join failed on GameServer. Changing back to MasterServer. Msg: " + operationResponse.DebugMessage);
					if (operationResponse.ReturnCode == 32758)
					{
						DConsole.Log("Most likely the game became empty during the switch to GameServer.");
					}
				}
	...
	private void AddNewPlayer(int ID, PhotonPlayer player)
	{
		...
		{
			DConsole.LogError("Adding player twice: " + ID);
		}
	}
	...
	private void HandleEventLeave(int actorID, EventData evLeave)
	{
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
		{
			DConsole.Log("HandleEventLeave for player ID: " + actorID + " evLeave: " + evLeave.ToStringFull());
		}
		PhotonPlayer playerWithId = GetPlayerWithId(actorID);
		if (playerWithId == null)
		{
			DConsole.LogError($"Received event Leave for unknown player ID: {actorID}");
			return;
		}
		...
		if (evLeave.Parameters.ContainsKey(233))
		{
			...
			if (playerWithId.IsInactive && isInactive)
			{
				DConsole.LogWarning("HandleEventLeave for player ID: " + actorID + " isInactive: " + playerWithId.IsInactive.ToString() + ". Stopping handling if inactive.");
				return;
			}
		}
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		if (PhotonNetwork.networkingPeer.State == ClientState.Disconnecting)
		{
			if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
			{
				DConsole.Log("OperationResponse ignored while disconnecting. Code: " + operationResponse.OperationCode);
			}
			return;
		}
		if (operationResponse.ReturnCode == 0)
		{
			if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
			{
				DConsole.Log(operationResponse.ToString());
			}
		}
		else if (operationResponse.ReturnCode == -3)
		{
			DConsole.LogError("Operation " + operationResponse.OperationCode + " could not be executed (yet). Wait for state JoinedLobby or ConnectedToMaster and their callbacks before calling operations. WebRPCs need a server-side configuration. Enum OperationCode helps identify the operation.");
		}
		else if (operationResponse.ReturnCode == 32752)
		{
			DConsole.LogError("Operation " + operationResponse.OperationCode + " failed in a server-side plugin. Check the configuration in the Dashboard. Message from server-plugin: " + operationResponse.DebugMessage);
		}
		else if (operationResponse.ReturnCode == 32760)
		{
			DConsole.LogWarning("Operation failed: " + operationResponse.ToStringFull());
		}
		else
		{
			DConsole.LogError("Operation failed: " + operationResponse.ToStringFull() + " Server: " + Server);
		}
		...
		case 231:
			if (operationResponse.ReturnCode != 0)
			{
				...
				{
					DConsole.LogError(string.Format("If you host Photon yourself, make sure to start the 'Instance LoadBalancing' " + base.ServerAddress));
				}
				else if (operationResponse.ReturnCode == short.MaxValue)
				{
					DConsole.LogError($"The appId this client sent is unknown on the server (Cloud). Check settings. If using the Cloud, check account.");
				...
				{
					DConsole.LogError($"Custom Authentication failed (either due to user-input or configuration or AuthParameter string format). Calling: OnCustomAuthenticationFailed()");
					SendMonoMessage(PhotonNetworkingMessage.OnCustomAuthenticationFailed, operationResponse.DebugMessage);
				}
				else
				{
					DConsole.LogError($"Authentication failed: '{operationResponse.DebugMessage}' Code: {operationResponse.ReturnCode}");
				}
				State = ClientState.Disconnecting;
				Disconnect();
				if (operationResponse.ReturnCode == 32757)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.LogWarning($"Currently, the limit of users is reached for this title. Try again later. Disconnecting");
					}
				...
				else if (operationResponse.ReturnCode == 32756)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.LogError($"The used master server address is not available with the subscription currently used. Got to Photon Cloud Dashboard or change URL. Disconnecting.");
					}
				...
				else if (operationResponse.ReturnCode == 32753)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.LogError($"The authentication ticket expired. You need to connect (and authenticate) again. Disconnecting.");
					}
		...
		case 220:
		{
			...
			if (operationResponse.ReturnCode != 0)
			{
				DConsole.LogError("GetRegions failed. Can't provide regions list. Error: " + operationResponse.ReturnCode + ": " + operationResponse.DebugMessage);
				break;
			}
			...
			{
				DConsole.LogError("The region arrays from Name Server are not ok. Must be non-null and same length. " + (array3 == null) + " " + (array4 == null) + "\n" + operationResponse.ToStringFull());
				break;
			}
			...
			for (int j = 0; j < array3.Length; j++)
			{
				...
				if (PhotonNetwork.PhotonServerSettings.HostType == ServerSettings.HostingOption.BestRegion && PhotonNetwork.PhotonServerSettings.EnabledRegions != 0)
				{
					...
					{
						DConsole.Log("Skipping region because it's not in PhotonServerSettings.EnabledRegions: " + cloudRegionCode);
					}
				}
				if (flag)
				{
			...
			{
				DConsole.Log("Best region found in PlayerPrefs. Connecting to: " + bestFromPrefs);
		...
		case 227:
		{
			...
			if (operationResponse.ReturnCode != 0)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
				{
					DConsole.LogWarning($"CreateRoom failed, client stays on masterserver: {operationResponse.ToStringFull()}.");
				}
		...
		case 226:
			if (Server != ServerConnection.GameServer)
			{
				if (operationResponse.ReturnCode != 0)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.Log($"JoinRoom failed (room maybe closed by now). Client stays on masterserver: {operationResponse.ToStringFull()}. State: {State}");
					}
					SendMonoMessage(PhotonNetworkingMessage.OnPhotonJoinRoomFailed, operationResponse.ReturnCode, operationResponse.DebugMessage);
				}
				else
		...
		case 225:
			if (operationResponse.ReturnCode != 0)
			{
				if (operationResponse.ReturnCode == 32760)
				{
					...
					{
						DConsole.Log("JoinRandom failed: No open game. Calling: OnPhotonRandomJoinFailed() and staying on master server.");
					}
				}
				else if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
				{
					DConsole.LogWarning($"JoinRandom failed: {operationResponse.ToStringFull()}.");
				}
				SendMonoMessage(PhotonNetworkingMessage.OnPhotonRandomJoinFailed, operationResponse.ReturnCode, operationResponse.DebugMessage);
			}
			else
		...
		case 222:
		{
			...
			{
				DConsole.LogError("FindFriends failed to apply the result, as a required value wasn't provided or the friend list length differed from result.");
			}
		...
		case 219:
			...
			break;
		default:
			DConsole.LogWarning($"OperationResponse unhandled: {operationResponse.ToString()}");
			break;
		case 252:
		case 253:
			break;
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
		{
			DConsole.Log($"OnStatusChanged: {statusCode.ToString()} current State: {State}");
		}
		switch (statusCode)
		{
		case StatusCode.Connect:
			if (State == ClientState.ConnectingToNameServer)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
				{
					DConsole.Log("Connected to NameServer.");
				}
			...
			if (State == ClientState.ConnectingToGameserver)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
				{
					DConsole.Log("Connected to gameserver.");
				}
			...
			if (State == ClientState.ConnectingToMasterserver)
			{
				if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
				{
					DConsole.Log("Connected to masterserver.");
				}
			...
			{
				DConsole.Log("Skipping EstablishEncryption. Protocol is secure.");
			}
		...
		case StatusCode.EncryptionEstablished:
			...
			{
				DConsole.Log("didAuthenticate " + didAuthenticate.ToString() + " AuthMode " + AuthMode);
			}
		...
		{
			DConsole.LogError(string.Concat("Encryption wasn't established: ", statusCode, ". Going to authenticate anyways."));
		...
		case StatusCode.Exception:
			if (IsInitialConnect)
			{
				DConsole.LogError("Exception while connecting to: " + base.ServerAddress + ". Check if the server is available.");
				if (base.ServerAddress == null || base.ServerAddress.StartsWith("127.0.0.1"))
				{
					DConsole.LogWarning("The server address is 127.0.0.1 (localhost): Make sure the server is running on this machine. Android and iOS emulators have their own localhost.");
					if (base.ServerAddress == GameServerAddress)
					{
						DConsole.LogWarning("This might be a misconfiguration in the game server config. You need to edit it to a (public) address.");
					}
				}
				State = ClientState.PeerCreated;
				DisconnectCause disconnectCause = (DisconnectCause)statusCode;
		...
		case StatusCode.TimeoutDisconnect:
			if (IsInitialConnect)
			{
				if (!_isReconnecting)
				{
					DConsole.LogWarning(string.Concat(statusCode, " while connecting to: ", base.ServerAddress, ". Check if the server is available."));
					IsInitialConnect = false;
					DisconnectCause disconnectCause = (DisconnectCause)statusCode;
					SendMonoMessage(PhotonNetworkingMessage.OnFailedToConnectToPhoton, disconnectCause);
				}
		...
		case StatusCode.DisconnectByServerLogic:
			...
			break;
		default:
			DConsole.LogError("Received unknown status code: " + statusCode);
			break;
		}
	}
	...
	public void OnEvent(EventData photonEvent)
	{
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
		{
			DConsole.Log($"OnEvent: {photonEvent.ToString()}");
		}
		...
		case 209:
		{
			...
			if (photonView == null)
			{
				DConsole.LogWarning("Can't find PhotonView of incoming OwnershipRequest. ViewId not found: " + num);
				break;
			}
			if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
			{
				DConsole.Log(string.Concat("Ev OwnershipRequest ", photonView.ownershipTransfer, ". ActorNr: ", sender, " takes from: ", num2, ". local RequestedView.ownerId: ", photonView.ownerId, " isOwnerActive: ", photonView.isOwnerActive.ToString(), ". MasterClient: ", mMasterClientId, ". This client's player: ", PhotonNetwork.player.ToStringFull()));
			}
			switch (photonView.ownershipTransfer)
			{
			case OwnershipOption.Fixed:
				DConsole.LogWarning("Ownership mode == fixed. Ignoring request.");
				break;
			case OwnershipOption.Takeover:
				if (num2 == photonView.ownerId || (num2 == 0 && photonView.ownerId == mMasterClientId) || photonView.ownerId == 0)
				{
					...
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.LogWarning(string.Concat(photonView, " ownership transfered to: ", sender));
					}
		...
		case 210:
		{
			...
			if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
			{
				DConsole.Log("Ev OwnershipTransfer. ViewID " + array[0] + " to: " + array[1] + " Time: " + Environment.TickCount % 1000);
			}
			int viewID = array[0];
			int num3 = array[1];
		...
		case 203:
			...
			{
				DConsole.LogError(string.Concat("Error: Someone else(", photonPlayer, ") then the masterserver requests a disconnect!"));
				break;
			}
			if (_AsyncLevelLoadingOperation != null)
			{
		...
		case 207:
		{
			...
			if ((int)DebugOut >= 3)
			{
				DConsole.Log("Ev DestroyAll! By PlayerId: " + sender);
			}
		...
		case 204:
		{
			int num4 = (int)((ExitGames.Client.Photon.Hashtable)photonEvent[245])[(byte)0];
			...
			{
				DConsole.LogError("Ev Destroy Failed. Could not find PhotonView with instantiationId " + num4 + ". Sent by actorNr: " + sender);
			}
			break;
		}
		case 208:
		...
		case 251:
			...
			{
				DConsole.LogWarning("Warning: Unhandled Event ErrorInfo (251). Set PhotonNetwork.OnEventCall to the method PUN should call for this event.");
			}
			break;
		case 223:
			if (AuthValues == null)
		...
		default:
			...
			{
				DConsole.LogWarning(string.Concat("Warning: Unhandled event ", photonEvent, ". Set PhotonNetwork.OnEventCall."));
			}
			break;
		}
	}
	...
	private void SetupEncryption(Dictionary<byte, object> encryptionData)
	{
		...
		{
			DConsole.LogWarning("SetupEncryption() called but ignored. Not XB1 compiled. EncryptionData: " + encryptionData.ToStringFull());
			return;
		}
		if (DebugOut == DebugLevel.INFO)
		{
			DConsole.Log("SetupEncryption() got called. " + encryptionData.ToStringFull());
		}
	...
	{
		DConsole.Log("SendVacantViewIds()");
		...
		DConsole.Log("Sending vacant view IDs. Length: " + list.Count);
	...
	protected internal void ExecuteRpc(ExitGames.Client.Photon.Hashtable rpcData, int senderID = 0)
	{
		...
		{
			DConsole.LogError("Malformed RPC; this should never occur. Content: " + SupportClass.DictionaryToString(rpcData));
			return;
		}
		...
		if (rpcData.ContainsKey(keyByteFive))
		{
			...
			{
				DConsole.LogError("Could not find RPC with index: " + num3 + ". Going to ignore! Check PhotonServerSettings.RpcList");
				return;
			}
		...
		if (photonView == null)
		{
			...
			if (flag)
			{
				DConsole.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! View was/is ours." + (flag2 ? " Owner called." : " Remote called.") + " By: " + senderID);
			}
			else
			{
				DConsole.LogWarning("Received RPC \"" + text + "\" for viewID " + num + " but this PhotonView does not exist! Was remote PV." + (flag2 ? " Owner called." : " Remote called.") + " By: " + senderID + " Maybe GO was destroyed but RPC not cleaned up.");
			}
			return;
		}
		if (photonView.prefix != num2)
		{
			DConsole.LogError("Received RPC \"" + text + "\" on viewID " + num + " with a prefix of " + num2 + ", our prefix is " + photonView.prefix + ". The RPC has been ignored.");
			return;
		}
		...
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			DConsole.Log("Received RPC: " + text);
		}
		...
		for (int j = 0; j < photonView.RpcMonoBehaviours.Length; j++)
		{
			...
			{
				DConsole.LogError("ERROR You have missing MonoBehaviours on your gameobjects!");
				continue;
			}
		...
		if (num6 == 0)
		{
			if (num7 == 0)
			{
				DConsole.LogError("PhotonView with ID " + num + " has no method \"" + text + "\" marked with the [PunRPC](C#) or @PunRPC(JS) property! Args: " + text2);
			}
			else
			{
				DConsole.LogError("PhotonView with ID " + num + " has no method \"" + text + "\" that takes " + array2.Length + " argument(s): " + text2);
			}
		}
		else
		{
			DConsole.LogError("PhotonView with ID " + num + " has " + num6 + " methods \"" + text + "\" that takes " + array2.Length + " argument(s): " + text2 + ". Should be just one?");
		}
	}
	...
	internal GameObject DoInstantiate(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
	{
		...
		if (resourceGameObject == null)
		{
			...
			if (resourceGameObject == null)
			{
				DConsole.LogError("PhotonNetwork error: Could not Instantiate the prefab [" + text + "]. Please verify you have this gameobject in a Resources folder.");
				return null;
			}
		}
	...
	public void DestroyPlayerObjects(int playerId, bool localOnly)
	{
		if (playerId <= 0)
		{
			DConsole.LogError("Failed to Destroy objects of playerId: " + playerId);
			return;
		}
		if (!localOnly)
		{
	...
	protected internal void RemoveInstantiatedGO(GameObject go, bool localOnly)
	{
		if (go == null)
		{
			DConsole.LogError("Failed to 'network-remove' GameObject because it's null.");
			return;
		}
		PhotonView[] componentsInChildren = go.GetComponentsInChildren<PhotonView>(includeInactive: true);
		if (componentsInChildren == null || componentsInChildren.Length == 0)
		{
			DConsole.LogError("Failed to 'network-remove' GameObject because has no PhotonView components: " + go);
			return;
		}
		...
		if (!localOnly)
		{
			...
			{
				DConsole.LogError("Failed to 'network-remove' GameObject. Client is neither owner nor masterClient taking over for owner who left: " + photonView);
				return;
			}
			if (instantiationId < 1)
			{
				DConsole.LogError(string.Concat("Failed to 'network-remove' GameObject because it is missing a valid InstantiationId on view: ", photonView, ". Not Destroying GameObject or PhotonViews!"));
				return;
			}
		}
		if (!localOnly)
		...
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			DConsole.Log("Network destroy Instantiated GO: " + go.name);
		}
		if (ObjectPool != null)
		{
	...
	private void ServerCleanInstantiateAndDestroy(int instantiateId, int creatorId, bool isRuntimeInstantiated)
	{
		...
		if (!isRuntimeInstantiated)
		{
			...
			DConsole.Log("Destroying GO as global. ID: " + instantiateId);
		}
	...
	{
		DConsole.Log("RequestOwnership(): " + viewID + " from: " + fromOwner + " Time: " + Environment.TickCount % 1000);
	...
	{
		DConsole.Log("TransferOwnership() view " + viewID + " to: " + playerID + " Time: " + Environment.TickCount % 1000);
		...
		if (value == null)
		{
			...
			foreach (PhotonView photonView in array)
			{
				if (photonView.viewID == viewID)
				{
					...
					{
						DConsole.LogWarning("Had to lookup view that wasn't in photonViewList: " + photonView);
					}
					return photonView;
				}
			}
	...
	public void RegisterPhotonView(PhotonView netView)
	{
		...
		if (netView.viewID == 0)
		{
			DConsole.Log("PhotonView register is ignored, because viewID is 0. No id assigned yet to: " + netView);
			return;
		}
		PhotonView value = null;
		if (photonViewList.TryGetValue(netView.viewID, out value))
		{
			...
			DConsole.LogError($"PhotonView ID duplicate found: {netView.viewID}. New: {netView} old: {value}. Maybe one wasn't destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.");
			RemoveInstantiatedGO(value.gameObject, localOnly: true);
		}
		photonViewList.Add(netView.viewID, netView);
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			DConsole.Log("Registered PhotonView: " + netView.viewID);
		}
	}
	...
	public void CleanRpcBufferIfMine(PhotonView view)
	{
		...
		{
			DConsole.LogError(string.Concat("Cannot remove cached RPCs on a PhotonView thats not ours! ", view.owner, " scene: ", view.isSceneView.ToString()));
		}
		else
		{
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, PhotonPlayer player, bool encrypt, params object[] parameters)
	{
		...
		if (view.viewID < 1)
		{
			DConsole.LogError("Illegal view ID:" + view.viewID + " method: " + methodName + " GO:" + view.gameObject.name);
		}
		if (PhotonNetwork.logLevel >= PhotonLogLevel.Full)
		{
			DConsole.Log(string.Concat("Sending RPC \"", methodName, "\" to target: ", target, " or player:", player, "."));
		}
		...
		default:
			DConsole.LogError("Unsupported target enum: " + target);
			break;
		}
	}
		...
		if (disableGroups != null)
		{
			...
			else
			{
				foreach (byte b in disableGroups)
				{
					if (b <= 0)
					{
						DConsole.LogError("Error: PhotonNetwork.SetInterestGroups was called with an illegal group number: " + b + ". The group number should be at least 1.");
					}
		...
		if (enableGroups != null)
		{
			...
			else
			{
				foreach (byte b3 in enableGroups)
				{
					if (b3 <= 0)
					{
						DConsole.LogError("Error: PhotonNetwork.SetInterestGroups was called with an illegal group number: " + b3 + ". The group number should be at least 1.");
					}
					else
					{
	...
	public void NewSceneLoaded()
	{
		...
		{
			DConsole.Log("New level loaded. Removed " + list.Count + " scene view IDs from last level.");
		}
	}
	...
	public void RunViewUpdate()
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (value == null)
			{
				DConsole.LogError($"PhotonView with ID {enumerator.Current.Key} wasn't properly unregistered! Please report this case to developer@photonengine.com");
				if (list == null)
				{
					list = new List<int>(4);
				}
	...
	private void OnSerializeRead(object[] data, PhotonPlayer sender, int networkTime, short correctPrefix)
	{
		...
		if (photonView == null)
		{
			DConsole.LogWarning("Received OnSerialization for view ID " + num + ". We have no such PhotonView! Ignored this if you're leaving a room. State: " + State);
		}
		else if (photonView.prefix > 0 && correctPrefix != photonView.prefix)
		{
			DConsole.LogError("Received OnSerialization for view ID " + num + " with prefix " + correctPrefix + ". Our prefix is " + photonView.prefix);
		}
		else
		{
			if (photonView.group != 0 && !allowedReceivingGroups.Contains(photonView.group))
			...
			if (photonView.synchronization == ViewSynchronization.ReliableDeltaCompressed)
			{
				...
				if (array == null)
				{
					if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
					{
						DConsole.Log("Skipping packet for " + photonView.name + " [" + photonView.viewID + "] as we haven't received a full packet for delta compression yet. This is OK if it happens for the first few frames after joining a game.");
					}
					return;
				}
	...
	protected internal void SetLevelInPropsIfSynced(object levelId, bool initiatingCall, bool asyncLoading = false)
	{
		...
		if (levelId == null)
		{
			DConsole.LogError("Parameter levelId can't be null!");
			return;
		}
		...
		{
			DConsole.LogError("Parameter levelId must be int or string!");
		}
		if (asyncLoading)
		{
